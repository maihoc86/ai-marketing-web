  ...
  ({
    ...
    return (
      <div
        ...
        ${
          isFeatured
            ? "bg-linear-to-br from-blue-600 via-blue-600 to-blue-700 text-white shadow-2xl shadow-blue-600/25 scale-[1.02] md:-mt-4 md:mb-4 z-10"
        ...
        <div className="p-8 flex flex-col h-full">
          ...
          <div className="min-h-[90px] flex flex-col justify-center mb-6">
            ...
            <div className="h-[50px] flex items-baseline gap-1">
              ...
              {!isContact && (
                <span
                  className={`text-sm font-medium ${isFeatured ? "text-blue-200" : "text-gray-400"}`}
                >
                  VNĐ
                </span>
              )}
            ...
            {!isContact && (
              <div
                className={`text-sm ${isFeatured ? "text-blue-200" : "text-gray-400"}`}
              >
                /{" "}
                {billing === "monthly"
                  ? "tháng"
          ...
          <ul className="space-y-3 flex-grow">
            {plan.features.map((feature, idx) => (
              <li key={idx} className="flex items-start gap-3">
                ...
                >
                  <Check
                    className={`w-3 h-3 ${isFeatured ? "text-white" : "text-blue-600"}`}
                    strokeWidth={3}
                  />
                </div>
                <span
                  className={`text-sm leading-relaxed ${isFeatured ? "text-blue-50" : "text-gray-600"}`}
                >
                  {feature}
                </span>
              </li>
              ...
              >
                ...
                <div className="min-h-[100px] flex flex-col justify-center mb-6">
                  <div className="h-[48px] flex items-baseline justify-start transition-opacity duration-200">
                    <span
                      className={`text-3xl md:text-4xl font-bold ${isFeatured ? "text-white" : "text-gray-900"}`}
                    >
                      {displayPrice}
                    </span>
                    {!isContact && (
                      <span
                        className={`text-base ml-2 ${isFeatured ? "text-gray-300" : "text-gray-500"}`}
                      >
                        VNĐ
                      </span>
                    )}
                  ...
                  <ul className="space-y-3">
                    {features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                        ...
                        />
                        <span
                          className={`text-sm leading-relaxed ${isFeatured ? "text-gray-300" : "text-gray-600"}`}
                        >
                          {feature}
                        </span>
                      </li>
