...
export function ProductEnterprise() {
  ...
  useEffect(() => {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (entry.target === technologyHeaderRef.current)
              setIsTechnologyHeaderVisible(true);
          }
        });
      },
      { threshold: 0.1 },
    ...
    if (technologyHeaderRef.current)
      observer.observe(technologyHeaderRef.current);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {aiModels.map((model, index) => (
              <AIModelCard
                ...
                model={model}
                index={index}
                isHeaderVisible={isTechnologyHeaderVisible}
              />
            ))}
          </div>
        </div>
...
function WhyDxaiCard({
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <h4 className="text-lg font-bold text-[#111518] mb-3 leading-tight">
        {t(feature.titleKey)}
      </h4>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div
          className={`w-10 h-10 ${feature.lightBg} rounded-lg flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300`}
        >
          <feature.icon className={`w-5 h-5 ${feature.color}`} />
        </div>
        <div className="flex-1 min-w-0">
          <h5 className="font-bold text-[#111518] text-sm mb-1 leading-tight">
            {t(feature.titleKey)}
          </h5>
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border-2 border-gray-200 rounded-3xl shadow-2xl overflow-hidden">
        ...
        {/* Dashboard Content */}
        <div className="p-6 bg-linear-to-br from-gray-50 to-[#22b5f8]/5">
          <div className="space-y-5">
            {/* Header with user */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-linear-to-br from-[#22b5f8] to-[#5fffec] rounded-xl flex items-center justify-center shadow-lg">
              ...
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-linear-to-br from-emerald-400 to-emerald-600 rounded-full" />
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-md border border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                <span className="px-3 py-1.5 bg-linear-to-r from-[#22b5f8] to-[#008bff] text-white rounded-lg text-xs font-bold shadow-sm">
                  Video AI
                </span>
                <span className="px-3 py-1.5 bg-linear-to-r from-purple-500 to-purple-600 text-white rounded-lg text-xs font-bold shadow-sm">
                  Content AI
                </span>
                <span className="px-3 py-1.5 bg-linear-to-r from-emerald-500 to-emerald-600 text-white rounded-lg text-xs font-bold shadow-sm">
                  Image AI
                </span>
                <span className="px-3 py-1.5 bg-linear-to-r from-amber-500 to-amber-600 text-white rounded-lg text-xs font-bold shadow-sm">
                  Chatbot
                </span>
              </div>
            </div>
      ...
      <div className="absolute -bottom-6 -left-6 bg-white rounded-xl p-4 shadow-2xl border-2 border-gray-100">
        ...
        <p className="text-sm font-bold text-gray-700">
          {t("about.product.stat.businesses")}
        </p>
      </div>
    </div>
  );
...
function AIModelCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl bg-linear-to-r from-transparent via-white to-transparent opacity-0 group-hover:opacity-20 group-hover:animate-pulse pointer-events-none" />
    </div>
  );
}
